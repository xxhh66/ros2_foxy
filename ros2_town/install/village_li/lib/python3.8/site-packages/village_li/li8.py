...
class BaiPiaoNode(Node):
    def __init__(self, name):
        ...
        # 发布消息类型Novel和订阅消息类型String不一致，导致无法接收消息
        # self.sub_novel = self.create_subscription(String,"sexy_girl",self.recv_novel_callback,10)
        # 修改为自定义消息类型Novel
        self.sub_novel = self.create_subscription(Novel,"sexy_girl",self.recv_novel_callback,10)
    ...
    def recv_novel_callback(self, novel):
        ...
        self.get_logger().info('李三：我收到了小说内容："%s"' % novel.content)
